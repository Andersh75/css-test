(function () {
	...
	*/
	...
	function setUserDropdownListener() {
	  const userAvatar = jquery(`.header__avatar`);

	  userAvatar.on(`click`, function(e) {
	    const dropdown = jquery(this).children(`.dropdown`);
	...
	function setSidenavListeners() {
	  const subHeadings = jquery(`x-navlist-subheading`);
	  console.log(`subHeadings: `, subHeadings);
	  const SUBHEADING_OPEN_CLASS = `x-navlist-subheading--open`;
	  const SUBLIST_HIDDEN_CLASS = `subList--hidden`;

	  subHeadings.each((i, subHeadingEl) => {
	    jquery(subHeadingEl).on(`click`, e => {
	      const subListEl = jquery(subHeadingEl).siblings();

	      // Add/remove selected styles to list category heading
	      if (subHeadingEl) {
	        toggleClass(jquery(subHeadingEl), SUBHEADING_OPEN_CLASS);
	      }

	      // Reveal/hide the sublist
	      if (subListEl && subListEl.length === 1) {
	        toggleClass(jquery(subListEl), SUBLIST_HIDDEN_CLASS);
	      }
	    });
	  });
	}
	...
	function setSidenavCloseListener() {
	  jquery(`.sidenav__brand-close`).on(`click`, function(e) {
